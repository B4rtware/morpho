...
class QDS_ECHO(dtaservice_pb2_grpc.DTAServerServicer):
    def TransformDocument(self, request: dtaservice_pb2.DocumentRequest, context):
        ...
        return dtaservice_pb2.TransformDocumentResponse(
            trans_document=l, trans_output=l.decode(), error=None
        )
...
def main():
    ...
    dts = pb.DocTransServer(
        RegistrarURL="http://127.0.0.1:8761/eureka", PortToListen="50051"
    )
    ...
    sa = QDS_ECHO()
    dtaservice_pb2_grpc.add_DTAServerServicer_to_server(sa, server)
    server.start()
